...
PROJECT_NAME=livemap
STACK_NAME=livemap-glicksoftware-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://livemap.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t davisagli/livemap-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t davisagli/livemap-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name livemap-frontend-acceptance --link livemap-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d davisagli/livemap-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name livemap-backend-acceptance -d davisagli/livemap-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop livemap-frontend-acceptance
	@docker stop livemap-backend-acceptance
