...
2. Go to your repository at [davisagli/livemap](https://github.com/davisagli/livemap).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `livemap.glicksoftware.com` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | livemap.glicksoftware.com | The hostname or IP address of your Docker Swarm manager. |
...
| Name     | Value |
|----------|-------|
| LIVE_ENV | livemap.glicksoftware.com |
...
- [Backend Tests Workflow](https://github.com/davisagli/livemap/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/davisagli/livemap/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/davisagli/livemap-backend`) and Frontend (`ghcr.io/davisagli/livemap-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of livemap.glicksoftware.com](https://github.com/davisagli/livemap/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/livemap.glicksoftware.com.yml` stack.
